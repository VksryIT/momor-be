...
const getHashedPasswordAndSalt = async (password) => {
    ...
    const hashedPassword = (await pbkdf2Promise(password, salt, 102542, 64, "sha512")).toString('base64');
...
const createUser = async (userInfo) => {
    let conn;
    try {
        const { hashedPassword, salt } = await getHashedPasswordAndSalt(userInfo.password);
        conn = await connectionPool.getConnection();
        await conn.beginTransaction();
        const userInsertResult = await conn.execute(`INSERT INTO user (name) VALUES ('${userInfo.name}')`);
